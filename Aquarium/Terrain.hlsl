...
PSINPUT RenderTerrainVS( VSINPUT input )
{
	...
	output.clip = dot( mul( input.position, mWorld ), fClipPlane );

	return output;
}

[maxvertexcount(3)]
void RenderTerrainGS( triangle PSINPUT input[3], inout TriangleStream<PSINPUT> OutputStream )
{	
    PSINPUT output = (PSINPUT)0;
...
WATER_PSINPUT RenderWaterVS( VSINPUT input )
{
	...
	output.position = mul( input.position, mModelViewProjectionMatrix );
	reflectProjectWorld = mul( mReflectionMatrix, mProjection );

	viewProjectWorld = mul( mView, mProjection );
	viewProjectWorld = mul( mWorld, viewProjectWorld );

	output.texcoord = input.texcoord / 0.01;
	output.texcoord2 = input.texcoord / 0.02;
...
//	//reflectTexCoord = reflectTexCoord + ( microbump_normal.xy * 0.01 );
//	
//	reflection_color = ReflectionTexture.Sample( samGeneral, reflectTexCoord );
//
...
//	final_color = saturate( water_color + 0.2 * reflection_color );
//	final_color.rgb += WaterSpecularIntensity * specular_factor * WaterSpecularColor * fresnel_factor;
...
float4 RenderWaterPS( WATER_PSINPUT input ) : SV_Target
{
	...
	normal = microbump_normal * 2.0f - 1.0f;
	...
	reflected_eye_to_pixel_vector = -pixel_to_eye_vector + 2 * dot( pixel_to_eye_vector, input.normal ) * input.normal;
	specular_factor = shadow_factor * /*fresnel_factor*/1.0f * pow( max( 0, dot( pixel_to_light_vector, reflected_eye_to_pixel_vector ) ), WaterSpecularPower );
	...
	reflectTexCoord.x = input.reflectionPosition.x / input.reflectionPosition.w / 2.0f + 0.5f;
	reflectTexCoord.y = -input.reflectionPosition.y / input.reflectionPosition.w / 2.0f + 0.5f;
	reflectTexCoord = reflectTexCoord + ( normal.xy * 0.03 );
	...
	//final_color.rgb += WaterSpecularIntensity * specular_factor * WaterSpecularColor * fresnel_factor;
	final_color = saturate( water_color + 0.2 * reflection_color );
	final_color.rgb += WaterSpecularIntensity * specular_factor * WaterSpecularColor * /*fresnel_factor*/1.0f;
	final_color = saturate( final_color );
